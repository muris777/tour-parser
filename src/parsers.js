...
- rigatrips-website emails (from our own site) have format: "From: [Name]Message: ...Number of Tickets: N...Departure Date: [Month] [Day], [Year] [HH:MM]h". Extract name from "From:", people count from "Number of Tickets:", date+time from "Departure Date:". The "Message:" field is a casual customer note — ignore any mentions of other people/friends in it, it does NOT mean multiple bookings. There is no booking number, no email, no phone for these — leave bn, e, ph as null. action=confirmed always for these unless message explicitly says cancel. Time is ALWAYS present in "Departure Date:" (the HH:MM before "h") — extract it even if formatting looks unusual, never leave time null for these emails. Map the tour name in the email to exactly one of these internal_code values, regardless of capitalization or wording variations: "Riga Old Town Free Tour" (any "Old Town"/"Riga Old Town" walking tour mention), "Latvian Food Tour" (any "Food Tour"/"Food Tasting Tour" mention), "KGB Soviet Tour" (any "Communist Riga"/"Soviet"/"KGB" mention), "Riga Old Town Walking Tour" (only if explicitly a paid/non-free walking tour). Language defaults to English unless another language is explicitly stated.
