...
Name: always extract the customer's FULL name — first name AND last/family name together as one string — never just the first name, even if the surname sits on a separate line or after other text before the next label. If the email writes the name in ALL CAPITAL LETTERS, still extract it complete and unchanged (do not stop after the first word, and do not bother re-casing it — leave the capitalization exactly as written).
...
Latvian Food Tour time correction: this tour's real start time is 13:00 (changed from an older 12:30 slot). Some providers/emails still reference the old 12:30 time even though the actual departure is 13:00. Whenever ic="Latvian Food Tour" and the extracted time is 12:30, output 13:00 instead — 12:30 and 13:00 both mean the same 13:00 departure for this tour.
Riga Old Town Free Tour time correction (English/Spanish only): this tour's start time depends on the booking's DATE, since it changes on 2026-10-01. For a booking dated before 2026-10-01, the correct time is 11:00 — whenever ic="Riga Old Town Free Tour", language is English or Spanish, and the extracted time is 10:30, output 11:00 instead. For a booking dated 2026-10-01 or later, the correct time reverts to 10:30 — whenever ic="Riga Old Town Free Tour", language is English or Spanish, and the extracted time is 11:00, output 10:30 instead. Always normalize to whichever of 10:30/11:00 is correct for that booking's own date, regardless of which one the provider's email states.
15:00 English "Walking Tour" alias: at 15:00 in English specifically, OTAs (e.g. Viator's "Riga Old Town walking tour", GetYourGuide's "Riga: Guided Old Town Walking Tour") resell the Riga Old Town Free Tour under a paid product name — there is no separate Walking Tour product at that time slot. Whenever a booking names a "Walking Tour" with time=15:00 and language=English, set ic="Riga Old Town Free Tour", not "Riga Old Town Walking Tour". This alias applies only to the 15:00 English slot — Walking Tour bookings at other times/languages are the genuine separate paid tour and keep ic="Riga Old Town Walking Tour".
...
- Freetour NO-SHOW (ignore): if the same cancellation email's "Reason:" line says "No show: automatically" (or any "No show" reason), this is Freetour's automated notice sent after the tour already happened because the guest simply didn't attend — not a genuine cancellation to act on, since a no-show is already handled on our end. Set action=unknown for these.
...
- Viator "Refund confirmation" emails (subject "Refund confirmation", body "Your refund was processed") are ALSO a cancellation — set action=cancelled for these too, even though the word "confirmation" appears and there's no "Canceled" text. Booking reference follows "Booking reference number:" and already includes the "BR-" prefix here (unlike amendment emails) — extract it exactly as given, prefix included.
...
- Civitatis (civitatis.com) emails include their own "Internal code:" field — always use that literal text as ic verbatim (it's more reliable than inferring from the tour name), EXCEPT: internal code "Kemeri National park and Jurmala Day Trip" only actually applies when Hour=08:45 (that combined trip's only departure). If the Internal code says "Kemeri National park and Jurmala Day Trip" but Hour is anything else (e.g. 13:00), Civitatis has mislabeled it — that's really the standalone "Kemeri Tour" product, so set ic="Kemeri Tour" instead.
- Get Your Guide (getyourguide.com) emails have two DIFFERENT "language" mentions — do not confuse them: "Language:" under "Main customer" is the customer's own personal language (e.g. "Language: Danish") and must be IGNORED; "Tour language:" (e.g. "Tour language / English (Live tour guide)") is the actual language the tour runs in and is the one to extract. Booking reference follows "Reference number". Date and time appear together like "August 1, 2026 1:00 PM" — convert to 24h HH:MM. Traveler counts appear like "1 x Youth (Age 13 - 18)" / "4 x Adults (Age 19 - 99)" — parse with the same traveler-bucket rules as everything else. Name/email/phone are under "Main customer". Otherwise apply the general field rules above.
- GuruWalk (guruwalk.com) modification emails ("You have a booking modification on a tour" / "A walker has modified a booking") list a "Details of the previous booking" section and a "Details of the new booking" section, each with their own "Booking code". Set action=amended. Extract bn from the PREVIOUS booking's Booking code (that's the one already stored in our system) — NEVER the new booking's code, GuruWalk issues a fresh code for every modification so the new one won't exist yet. Extract date/time/language/attendees from the NEW booking section — those are the updated values that replace the old ones. GuruWalk always phrases its tour name as "[Tour] must-see free tour with a local" regardless of which actual product it is — this maps to internal_code "Riga Old Town Free Tour" unless the tour name also contains another ic's keywords (KGB/Soviet, Art Nouveau, Food Tour, etc.), in which case that other ic wins.
- GuruWalk NAME (common mistake, watch for this): the walker's name is often written as two separate words/lines (first name, then family name right after, sometimes on its own line or run together with the next label) — grab BOTH parts as one full name, not just the first word. GuruWalk frequently writes this name in ALL CAPS; extract the whole thing regardless.
